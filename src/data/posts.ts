...
const unsortedPosts: Post[] = [
  ...
  },
  {
    ...
    title: "The Leaderboard Changes Every Week, the Excitement Never Outlives It",
    date: "2026-09-12",
    excerpt:
      "An excitement built in an afternoon lasts only as long as a frontier model holds its ranking \u2014 switch every launch, and six times a year you start over as a beginner.",
    paragraphs: [
      "Yesterday, Anthropic, OpenAI, and xAI all launched their new flagship models, and within several hours benchmarks and performance tests appeared everywhere, showing Astra 6 better than Fable 5.1, and OpenAI back in front again. People, therefore, celebrated this \u2018historic\u2019 moment and saw it as karma for Dario being arrogant, and many announced that they were cancelling Claude and switching to other platforms. However, this kind of scenario seems to have appeared before. A few months ago it carried similarly, only with the victim being OpenAI. The target always changes but the discussion stays the same, and almost none of those who switched last time were still talking about it a month later.",
      "What actually matters here is not which side people pick, but how fast the enthusiasm arrives and drops, and how much that costs. An excitement built in an afternoon lasts only as long as a frontier model holds its ranking, which is one or two weeks on average, so the cycle keeps feeding itself: you switch, you get a few weeks of novelty, the next launch lands, and you switch again. Six times a year, you start over as a beginner.",
      "And this is the part missing from the celebration, as a tool only pays you back after the boring part, after you have learned where it fails, what it needs from your prompts, and which jobs to keep away from it. That knowledge is the actual asset, and it takes far longer to build than the hype takes to expire, so every switch throws it away and buys a fresh launch-day feeling instead. Meanwhile the number driving the whole thing measures a narrow, averaged version of capability, which often has little to do with the work you personally do, and the model that came second can still be the better one for your own field.",
      "So the message here is not that one lab is winning, nor that loyalty is a virtue. It is that the useful question was never which model is ahead this week, but which one is better at the specific work in front of you \u2014 and you'd only know that after staying with one long enough to stop being impressed by it. I use several, for different jobs, and none of them are exciting to me anymore. That is exactly the point, and I'd rather it stayed that way.",
      "I'm Richael. My two apps are Soulor, a multi-companion and simulation platform, and VNportal, a music recreation and creation community, and both of them care about which model is right for one narrow job, not which one is ahead this week. My last post, about why \u201cship before perfection\u201d is out of date, is here: https://dh8116.github.io/blog/ship-before-perfection-is-out-of-date. If you're interested, check out my website or follow for more, thanks for your support!",
    ],
  },
];
